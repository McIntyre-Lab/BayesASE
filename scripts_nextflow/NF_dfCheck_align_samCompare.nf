...
/*
*    Nextflow script for prepping ase counts data and running bayesian machine*
...
println """\
    ...
    scripts:            ${SCRIPTS}
    bedfile:            ${BEDFILE}
    design file:        ${DESIGN_HEADER}
    """
    .stripIndent()



DF = Channel.from( DESIGN_HEADER )
...
process checkDF {
    ...
    python !{SCRIPTS}/check_pre_aln_design_10amm.py \
        ...
        --ext .fq \
        --readLen readLength \
         --dups !{PROJ}/ase_check_summaries/design_file_list_duplicate_fqNames.csv \
        --logfile !{PROJ}/ase_check_summaries/design_file_check.log
    '''
}
...
process counts {
    ...
    do
        ...
        then
            ...
            echo -e "Aligning !{READ} to !{row.G1}"
