...
#setwd("C:/Users/lleonnovelo/OneDrive - The University of Texas Health Science Center at Houston/projects/RNAextended/Alison_questions/")
...
gam.mles.data = function(x){
  ...
  xb = mean(x)
  xd = mean(log(x))
  s = log(xb)-xd#s=0 when all the observations have the same value
  a0 = ifelse(s>0,(3.0-s+sqrt((s-3.0)^2+24.0*s))/12.0/s,50) #The second value is in case s=0,a0=50 corresponds to s=0.01, line changed 8/7/2024
  l = 1
  repeat{
...
#args<-c("../../for_luis_02/dm12272_01h_temp.csv",
#        "../../for_luis_02/dm12272_01h_temp_out.csv",1,
#        "C:/Users/lleonnovelo/OneDrive - The University of Texas Health Science Center at Houston/projects/RNAextended/Alison_questions/for_luis/stan2_scripts",
#        10^5,10^4)
...
#	q_= matrix(c(as.numeric(mydata["prior_c1_g1"]),as.numeric(mydata["prior_c1_g2"]),as.numeric(mydata["prior_c2_g1"]),as.numeric(mydata["prior_c2_g2"])) ,nrow=2, byrow=TRUE)  #Bias correction hyperparameter expected percentage of counts aligning to line when no AI
	q_= matrix(c(as.numeric(mydata[allpriors])) ,nrow=nconditions,ncol=2, byrow=TRUE)  #Bias correction hyperparameter expected percentage of counts aligning to line when no AI
...
#set.seed(123)	
    ...
    Stanresults[mystan,]<-c(mean(theta1),quantile(theta1,c(0.025,0.975)),
                  2*min(mean(theta1>1/2),mean(theta1<1/2)) )
#    Stanresults[mystan,]<-c(mean(theta1),quantile(theta1,c(0.025,0.975)),
