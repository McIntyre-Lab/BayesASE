...
parser.add_argument('-fq','--fq', dest='fq', action='store', required=True, help='FQ file used in alignment [Required]')
parser.add_argument('-o','--out', dest='out', action='store', required=True, help='Output file containing check info [Required]')
args = parser.parse_args()


## calculate number of starting reads in fq file
...
## open G2 aln summary file and output file
...
with open(args.out, 'w') as outfile:
    ...
    if first_row:
        spamwriter.writerow(['fqName', 'start_read_num', 'readNum_G1', 'readNum_G2', 'flag_start_readNum_eq_readNum_G1', 'flag_start_readNum_eq_readNum_G2'])
        first_row=False

        row_items = [name, start_reads, end_reads1, end_reads2, flag_start_readNum_eq_end_readNum_G1, flag_start_readNum_eq_end_readNum_G2]
